...
async function deletePfandCategory() {
  console.log('🚀 Pfand kategorisi siliniyor...\n');

  try {
    const pool = await sql.connect(mssqlConfig);
    console.log('✅ MSSQL bağlantısı başarılı\n');

    // Pfand kategorisini bul
    const categoryResult = await pool.request().query(`
      SELECT id, name FROM categories WHERE name = 'Pfand'
    `);

    if (categoryResult.recordset.length === 0) {
      console.log('ℹ️  Pfand kategorisi bulunamadı.');
      await pool.close();
      return;
    }
    ...
    `);
    console.log(`✓ Pfand kategorisi silindi`);
...
deletePfandCategory().catch(console.error);
