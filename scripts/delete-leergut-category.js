...
async function deleteLeergutCategory() {
  console.log('🚀 Leergut kategorisi siliniyor...\n');

  try {
    const pool = await sql.connect(mssqlConfig);
    console.log('✅ MSSQL bağlantısı başarılı\n');

    // Leergut kategorisini bul
    const categoryResult = await pool.request().query(`
      SELECT id, name FROM categories WHERE name = 'Leergut'
    `);

    if (categoryResult.recordset.length === 0) {
      console.log('ℹ️  Leergut kategorisi bulunamadı.');
      await pool.close();
      return;
    }
    ...
    `);
    console.log(`✓ Leergut kategorisi silindi`);
...
deleteLeergutCategory().catch(console.error);
