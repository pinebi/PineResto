...
async function deleteDiverseCategory() {
  console.log('🚀 Diverse kategorisi siliniyor...\n');

  try {
    const pool = await sql.connect(mssqlConfig);
    console.log('✅ MSSQL bağlantısı başarılı\n');

    // Diverse kategorisini bul
    const categoryResult = await pool.request().query(`
      SELECT id, name FROM categories WHERE name = 'Diverse'
    `);

    if (categoryResult.recordset.length === 0) {
      console.log('ℹ️  Diverse kategorisi bulunamadı.');
      await pool.close();
      return;
    }
    ...
    `);
    console.log(`✓ Diverse kategorisi silindi`);
...
deleteDiverseCategory().catch(console.error);
